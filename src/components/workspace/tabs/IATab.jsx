...
import { ArrowUp, Bot, User, Trash2, FolderPlus, Folder, ChevronLeft, X, Paperclip, Search, Settings, Pencil, Check, Zap, Code2, Copy, Send } from 'lucide-react';
...
export default function IATab({ onSendToGenerator }) {
  ...
  const handleNewChat = (projId = activeProjectId) => {
    ...
    const existingEmpty = sessions.find(s => s.projectId === projId && s.messages.length === 1 && !s.hasCustomTitle);
    if (existingEmpty) {
  ...
  const renderSessionItem = (session) => {
    ...
    if (isEditing) {
      return (
        <div key={session.id} className="w-full flex items-center px-2 py-1.5 bg-surface-container-lowest border border-outline-variant rounded-md shadow-sm">
          <input
            autoFocus
            type="text"
          ...
          />
          <button onClick={saveSessionTitle} className={`p-1 text-on-surface hover:bg-surface-container rounded-md shrink-0 ${FOCUS_RING}`}>
      ...
      <div className="w-64 bg-surface-container-lowest border border-surface-variant rounded-3xl flex flex-col shadow-sm flex-shrink-0 overflow-hidden">
        <div className="p-4">
          {activeProjectId === null ? (
            <button onClick={() => handleNewChat(null)} className={`w-full bg-primary text-on-primary hover:opacity-90 font-semibold text-[13px] py-2.5 rounded-lg transition-colors flex items-center justify-center gap-2 ${FOCUS_RING}`}>
              + Nouvelle discussion
            </button>
          ) : (
            <button onClick={leaveProject} className={`w-full bg-surface-container text-on-surface hover:bg-surface-variant font-semibold text-[13px] py-2 rounded-lg transition-colors flex items-center justify-center gap-1 ${FOCUS_RING}`}>
        ...
        <div className="px-4 pb-4 border-b border-surface-variant">
          <div className="relative">
            <Search className="w-3.5 h-3.5 text-on-surface-variant absolute left-3 top-1/2 -translate-y-1/2" aria-hidden="true" />
            <input
              type="text"
          ...
          {activeProjectId === null && (
            <div className="mb-6">
              <div className="flex items-center justify-between mb-2 px-2 mt-2">
                <span className="font-label-caps text-label-caps text-on-surface-variant">Mes Projets</span>
                <button onClick={openCreateModal} className={`text-on-surface-variant hover:text-primary transition-colors rounded ${FOCUS_RING}`} title="Nouveau projet" aria-label="Nouveau projet"><FolderPlus className="w-3.5 h-3.5" aria-hidden="true" /></button>
              </div>
              {projects.length === 0 ? (
                ...
                projects.map(proj => (
                  <div key={proj.id} className="relative group mb-1">
                    <button onClick={() => enterProject(proj.id)} className={`w-full text-left px-3 py-2 text-[13px] font-semibold rounded-md truncate flex items-center gap-2 text-on-surface hover:bg-surface-container transition-colors border border-transparent group-hover:border-surface-variant ${FOCUS_RING}`}>
                      <Folder className="w-3.5 h-3.5" aria-hidden="true" style={{ color: proj.color || '#999' }} /> {proj.name}
                    </button>
                  </div>
                ))
              )}
              ...
              <div className="flex items-center justify-between mb-2 px-2 mt-6">
                <span className="font-label-caps text-label-caps text-on-surface-variant">Configuration</span>
              </div>
              <button onClick={openSkillModal} className={`w-full text-left px-3 py-2 text-[13px] font-semibold rounded-md truncate flex items-center gap-2 text-on-surface hover:bg-surface-container transition-colors border border-transparent hover:border-surface-variant ${FOCUS_RING}`}>
              ...
              <div className="bg-surface-container border border-surface-variant rounded-lg p-3">
                <div className="flex items-center justify-between gap-2 mb-1">
                  <div className="flex items-center gap-2 text-on-surface font-semibold text-sm truncate">
                    <Folder className="w-4 h-4" aria-hidden="true" style={{ color: projects.find(p => p.id === activeProjectId)?.color || '#999' }} />
                    <span className="truncate">{projects.find(p => p.id === activeProjectId)?.name}</span>
                  </div>
                  <button onClick={openEditModal} className={`text-on-surface-variant hover:text-primary transition-colors shrink-0 rounded ${FOCUS_RING}`} title="Paramètres du projet" aria-label="Paramètres du projet">
                    <Settings className="w-4 h-4" aria-hidden="true" />
                  </button>
                </div>
                <button onClick={() => handleNewChat(activeProjectId)} className={`w-full mt-2 bg-surface-container-lowest border border-surface-variant text-on-surface hover:bg-surface-container text-[12px] py-1.5 rounded-md transition-colors ${FOCUS_RING}`}>+ Nouveau chat</button>
              </div>
            </div>
          )}

          <span className="font-label-caps text-label-caps text-on-surface-variant mb-2 px-2 mt-2">Aujourd'hui</span>
          {recentSessions.length === 0 && <div className="text-xs text-on-surface-variant px-2 py-1">{searchQuery ? "Aucun résultat" : "Vide"}</div>}
          {recentSessions.map(renderSessionItem)}

          {olderSessions.length > 0 && (
            <>
              <span className="font-label-caps text-label-caps text-on-surface-variant mt-6 mb-2 px-2">Précédent</span>
      ...
      <div className="flex-1 bg-surface-container-lowest border border-surface-variant rounded-3xl shadow-sm flex flex-col relative overflow-hidden">
        {activeProjectId !== null ? (
          <div className="h-10 bg-surface-container border-b border-surface-variant flex items-center px-4 gap-2 text-xs text-on-surface-variant flex-shrink-0">
            <Folder className="w-3.5 h-3.5" aria-hidden="true" style={{ color: projects.find(p => p.id === activeProjectId)?.color || '#999' }} />
            <span>Chat dans le projet : <strong className="text-on-surface">{projects.find(p => p.id === activeProjectId)?.name}</strong></span>
          </div>
        ) : (
        ...
        <div className="flex-1 flex flex-col p-6 overflow-hidden relative">
          <div className="flex-1 overflow-y-auto pb-32 pr-4 flex flex-col gap-6">
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex gap-4 max-w-3xl min-w-0 ${msg.role === 'user' ? 'self-end flex-row-reverse' : 'self-start'}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${msg.role === 'user' ? 'bg-surface-container-high' : 'bg-primary text-on-primary'}`}>
                  {msg.role === 'user' ? <User className="w-5 h-5 text-on-surface" aria-hidden="true" /> : <Bot className="w-5 h-5" aria-hidden="true" />}
                </div>
                <div className={`p-5 rounded-2xl text-[15px] leading-relaxed break-words min-w-0 ${msg.role === 'user' ? 'bg-surface-container-high text-on-surface rounded-tr-sm' : 'bg-surface-container-lowest border border-surface-variant shadow-sm text-on-surface rounded-tl-sm w-full max-w-full overflow-hidden'}`}>
                  {msg.image && (
                    <div className="mb-3">
                      <img src={msg.image} alt="Upload" className="max-w-xs rounded-lg border border-surface-variant" />
                    </div>
                  )}
                  {msg.role === 'user' ? (
            ...
            {isLoading && (
              <div className="flex gap-4 max-w-3xl self-start">
                <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm bg-primary text-on-primary"><Bot className="w-5 h-5" aria-hidden="true" /></div>
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-3rem)] max-w-3xl flex flex-col gap-2 z-20">
            ...
            {showSlashMenu && filteredSlashCommands.length > 0 && (
              <div className="absolute bottom-full mb-2 w-full bg-surface-container-lowest border border-surface-variant rounded-xl shadow-lg p-2 flex flex-col gap-1 max-h-60 overflow-y-auto z-10">
                <div className="font-label-caps text-label-caps text-on-surface-variant px-2 py-1">Compétences / Skills</div>
                {filteredSlashCommands.map((cmd) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-[13px] text-on-surface">{cmd.title}</span>
                      <span className="text-[11px] text-on-surface-variant font-medium bg-surface-container px-1.5 py-0.5 rounded">{cmd.name}</span>
                    </div>
                    <span className="text-[11px] text-on-surface-variant line-clamp-1 mt-0.5">{cmd.instruction}</span>
                  </button>
                ))}
              </div>
            )}
            ...
            <div className="w-full bg-surface-container-lowest rounded-2xl border-2 border-outline-variant focus-within:border-primary shadow-[0_8px_30px_-10px_rgba(0,0,0,0.06)] flex flex-col p-2 transition-all duration-300">
              ...
              {activeSkill && (
                <div className="flex items-center gap-1.5 bg-surface-container text-on-surface px-2.5 py-1.5 rounded-lg text-[12px] font-semibold border border-surface-variant shadow-sm ml-2 mb-1 w-fit">
                  ...
                  <span className="font-normal text-on-surface-variant">({activeSkill.name})</span>
                  <button onClick={() => setActiveSkill(null)} className={`ml-1 text-on-surface-variant hover:text-on-surface transition-colors rounded-full p-0.5 hover:bg-surface-variant ${FOCUS_RING}`}>
              ...
              <div className="flex items-end w-full">
                ...
                <textarea
                  ...
                  placeholder={isLoading ? "L'IA réfléchit..." : activeSkill ? "Votre demande (la compétence sera appliquée automatiquement)..." : "Posez une question ou demandez un prompt..."}
                  disabled={isLoading}
                  rows={1}
            ...
            <div className="text-center">
              <span className="font-label-caps text-label-caps text-outline">L&apos;IA peut faire des erreurs. Vérifiez les prompts générés.</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="bg-surface-container-lowest rounded-3xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col">
            <div className="flex items-center justify-between p-5 border-b border-surface-variant">
              <h2 className="text-lg font-bold text-on-surface flex items-center gap-2">
                {editingProjectId ? <Settings className="w-5 h-5" aria-hidden="true" /> : <FolderPlus className="w-5 h-5" aria-hidden="true" />}
                {editingProjectId ? "Paramètres du Projet" : "Nouveau Projet"}
              </h2>
              <button onClick={() => setIsModalOpen(false)} aria-label="Fermer" className={`text-on-surface-variant hover:text-on-surface transition-colors rounded ${FOCUS_RING}`}><X className="w-5 h-5" aria-hidden="true" /></button>
            </div>
            <div className="p-6 flex flex-col gap-5">
              <div>
                <label className="block text-sm font-semibold text-on-surface mb-2">Nom du projet</label>
                <input type="text" value={newProjectName} onChange={e => setNewProjectName(e.target.value)} placeholder="Ex: Refonte Dashboard SaaS" className={`w-full bg-surface-container border border-transparent focus:border-primary rounded-lg px-4 py-2.5 text-sm outline-none transition-colors ${FOCUS_RING}`} />
              </div>
              ...
              <div>
                <label className="block text-sm font-semibold text-on-surface mb-1">Instructions Personnalisées</label>
                <p className="text-xs text-on-surface-variant mb-2">Définissez comment l'IA doit se comporter pour tous les chats de ce projet.</p>
                <textarea value={newProjectInstruction} onChange={e => setNewProjectInstruction(e.target.value)} placeholder="Ex: Tu es un expert en design minimaliste. Utilise toujours le tutoiement." className={`w-full bg-surface-container border border-transparent focus:border-primary rounded-lg px-4 py-3 text-sm outline-none transition-colors resize-none h-32 ${FOCUS_RING}`} />
              </div>
            </div>

            <div className="p-5 border-t border-surface-variant flex items-center justify-between bg-surface-container">
              {editingProjectId ? (
                <button onClick={handleDeleteProject} className={`flex items-center gap-2 text-sm font-semibold text-error hover:opacity-80 transition-opacity rounded ${FOCUS_RING}`}>
              ...
              <div className="flex gap-3">
                <button onClick={() => setIsModalOpen(false)} className={`px-5 py-2 rounded-lg text-sm font-semibold text-on-surface-variant hover:bg-surface-variant transition-colors ${FOCUS_RING}`}>Annuler</button>
                <button onClick={handleSaveProject} disabled={!newProjectName.trim()} className={`px-5 py-2 rounded-lg text-sm font-semibold bg-primary text-on-primary hover:opacity-90 transition-colors disabled:opacity-50 ${FOCUS_RING}`}>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-surface-container-lowest rounded-3xl shadow-xl w-full max-w-4xl overflow-hidden flex h-[80vh]">
            ...
            <div className="w-2/3 flex flex-col bg-surface-container-lowest relative">
              <button onClick={() => setIsSkillModalOpen(false)} aria-label="Fermer" className={`absolute top-4 right-4 p-2 text-on-surface-variant hover:text-on-surface transition-colors rounded-full hover:bg-surface-container z-10 ${FOCUS_RING}`}><X className="w-5 h-5" aria-hidden="true" /></button>
              ...
              <div className="flex-1 p-8 flex flex-col gap-6 overflow-y-auto">
                ...
                <div className="flex gap-4">
                  <div className="w-1/3">
                    <label className="block text-sm font-semibold text-on-surface mb-2">Raccourci (slash)</label>
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant font-bold">/</span>
                      <input type="text" value={newSkillTitle} onChange={e => setNewSkillTitle(e.target.value.replace(/[^a-zA-Z0-9_-]/g, '').toLowerCase())} placeholder="seo" className={`w-full bg-surface-container border border-transparent focus:border-primary rounded-lg pl-7 pr-4 py-2.5 text-sm outline-none transition-colors ${FOCUS_RING}`} />
                    </div>
                  </div>
                  <div className="w-2/3">
                    <label className="block text-sm font-semibold text-on-surface mb-2">Nom de la compétence</label>
                    <input type="text" value={newSkillName} onChange={e => setNewSkillName(e.target.value)} placeholder="Ex: Expert SEO & Copywriting" className={`w-full bg-surface-container border border-transparent focus:border-primary rounded-lg px-4 py-2.5 text-sm outline-none transition-colors ${FOCUS_RING}`} />
                  </div>
                </div>

                <div className="flex-1 flex flex-col">
                  <label className="block text-sm font-semibold text-on-surface mb-1">Instructions Invisibles (System Prompt)</label>
                  <p className="text-xs text-on-surface-variant mb-3">Ces instructions seront transmises à l'IA avec votre message de manière invisible lorsque vous utilisez ce raccourci.</p>
                  <textarea value={newSkillInstruction} onChange={e => setNewSkillInstruction(e.target.value)} placeholder="Ex: Agis comme un expert en référencement naturel. Ton but est d'optimiser le contenu suivant pour le mot-clé principal..." className={`w-full flex-1 min-h-[200px] bg-surface-container border border-transparent focus:border-primary rounded-lg px-4 py-3 text-sm outline-none transition-colors resize-none leading-relaxed ${FOCUS_RING}`} />
                </div>
              </div>

              <div className="p-6 border-t border-surface-variant flex items-center justify-between bg-surface-container">
                {editingSkillId ? (
                  <button onClick={() => handleDeleteSkill(editingSkillId)} className={`flex items-center gap-2 text-sm font-semibold text-error hover:opacity-80 transition-opacity rounded ${FOCUS_RING}`}>
                ...
                <div className="flex gap-3">
                  <button onClick={() => setIsSkillModalOpen(false)} className={`px-5 py-2.5 rounded-lg text-sm font-semibold text-on-surface-variant hover:bg-surface-variant transition-colors ${FOCUS_RING}`}>Fermer</button>
                  <button onClick={handleSaveSkill} disabled={!newSkillTitle.trim() || !newSkillName.trim() || !newSkillInstruction.trim()} className={`px-5 py-2.5 rounded-lg text-sm font-semibold bg-primary text-on-primary hover:opacity-90 transition-colors disabled:opacity-50 flex items-center gap-2 ${FOCUS_RING}`}>
                    <Check className="w-4 h-4" aria-hidden="true" /> {editingSkillId ? "Mettre à jour" : "Sauvegarder"}
                  </button>
                </div>
              </div>
            </div>
